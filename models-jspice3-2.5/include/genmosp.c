...
void
GENmosParse(type,ckt,tabp,currentp)
...
GENERIC *currentp;
{
    ...
    if (thismodel != NULL) {
        ntype = thismodel->INPmodType;
        if (ntype != type) {

            /* hack for shared key for MOS and BSIM */

            keys = ft_sim->devices[ntype]->keys;
            for (i = 0; i < *ft_sim->devices[ntype]->numKeys; i++) {
                if (keys[i].key == 'm')
                    break;
            }
            if (i == *ft_sim->devices[ntype]->numKeys) {
    ...
    if (waslead) {
        LITERR("No unlabeled parameter permitted on mosfet.")
    }
}
