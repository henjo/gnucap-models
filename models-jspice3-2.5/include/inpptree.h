...
typedef struct INPparseNode {
    ...
    int (*evfunc)();            /* called during evaluation */
    double (*function)();       /* ... and pointer to the function. */
    double *trancoeffs;         /* tran function parameters (as input). */
    double *tranparms;          /* tran function run time parameters. */
    double *trancache;          /* cached stuff for tran evaluation. */
    int numtrancoeffs;          /* number of tran parameters input. */
    int pwlindex;               /* index into pwl tran func array. */
    int new_deriv;              /* nonzero if created after parse. */
} INPparseNode;
...
/* The following things are used by the parser --
 * these are the token types the
 * lexer returns.
 */
...
extern struct sPTfunc PTfuncs[];
extern struct sPTfunc PTtFuncs[];
...
extern struct sPTop PTops[];

#ifdef __STDC__
extern int IFinit(IFparseTree *, double, double);
extern int IFeval(IFparseTree *, double, double*, double*, double*, double*);
extern void IFfree(IFparseTree *);
#else /* stdc */
extern int IFinit();
extern int IFeval();
extern void IFfree();
