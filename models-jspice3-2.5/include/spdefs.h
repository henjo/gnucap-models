...
/* SRW -- changed this so that if spCOMPLEX is defined and the matrix
 * is real, the imaginary part is ignored.  Then we can cache stuff
 * in the imaginary part without screwing up pivot selection.
 */
#if spCOMPLEX
/* original
#define  ELEMENT_MAG(ptr)   (ABS((ptr)->Real) + ABS((ptr)->Imag))
*/
#define  ELEMENT_MAG(ptr)   (Matrix->Complex ? \
    (ABS((ptr)->Real) + ABS((ptr)->Imag)) : \
...
{   RealNumber   Real;
    ...
    RealNumber   Imag;
#endif
    RealNumber   Init;  /* SRW, used to initialize */
    int          Row;
    int          Col;
